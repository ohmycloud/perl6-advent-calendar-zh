...
我们做到了！ 而现在，对于最后的格林奇 - 值得接触，我们将混淆数字计算的数值结果。 Rakudo 缓存 `Int` 常量。 当用不同类型的数字计算时，Infix `+` 运算符也使用 [internal-ish-ish](https://github.com/perl6/doc/issues/1690) `.Bridge` 方法。 所以，让我们重写常量上的 `.Bridge` 来返回一些奇怪的东西：
